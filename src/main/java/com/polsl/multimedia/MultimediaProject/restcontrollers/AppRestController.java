...
public class AppRestController {
    ...
    public ResponseEntity<PhotoParams> getParams(@RequestParam Long photoId, Authentication authentication){
        AppUser appUser = userService.getUserWithUsername(authentication.getName());
        if(appUser!=null){
            Photo photo = userService.getUsersPhoto(appUser, photoId);
    ...
    public HttpStatus uploadParams(@RequestBody PhotoParams photoParams, @RequestParam Long photoId, Authentication authentication){
        AppUser appUser = userService.getUserWithUsername(authentication.getName());
        if(appUser!=null){
            Photo photo = photoService.getPhotoWithId(photoId);
            if(userService.userHasPhoto(appUser, photo)){
                photoService.updatePhotoParams(photo, photoParams);
